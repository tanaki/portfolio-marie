...
$(lis[index]).animate({"width":"51.2%"},function(){var $span=$("span",this);$span.css("display","block");var marginTop=$span.height()>0?125-$span.height():135;$(this).animate({"margin-top":marginTop},function(){isAnimating=false;});});currentIndex=index;},reset:function(){if(isAnimating)return false;$("a.current",context).removeClass("current");$("li",context).each(function(i,el){$(el).animate({"width":"24.2%","margin-top":135},function(){currentIndex=null;isAnimating=false;$(window).trigger("NAV_RESET");});});},display:function(){var lis=$("#menu li");$("#header-left").fadeIn(300,function(){$("#copyright").fadeIn();lis.each(function(i,el){$(el).delay(i*100).fadeIn(200,function(){if(i==lis.length-1){$(window).trigger("NAV_DISPLAYED");}});});});}};$.fn.MLMenu=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.menu');}};})(jQuery);(function($){var isAnimatingTop=false,isAnimatingBottom=false;var methods={init:function(options){var self=this;},display:function(){$(".mosaic img").each(function(i,el){$(el).attr("src",$(el).data("src"));});$(window).MLResize("resize");var max=$(".line-container:first li").length-1;$(".mosaic li.project").each(function(i,el){var index=i>max?i-max:i;$(el).delay(index*120).fadeIn(200);});methods.hover();methods.carousel();},hover:function(){$(".mosaic ul a").hover(function(){var self=this;$('.keywords span',self).each(function(i,el){$(el).stop().delay(i*100).animate({"margin-left":"1"});});},function(){$('.keywords span',this).each(function(i,el){$(el).stop(true).delay(i*100).animate({"margin-left":"210"});});});},carousel:function(){$(".mosaic-prev-top").mouseover(function(){methods._prev(".line-container:first",-135,"top");});$(".mosaic-next-top").mouseover(function(){var max=$(window).width()-$(".line-container:first").width()+100;methods._next(".line-container:first",max,"top");});$(".mosaic-prev-bottom").mouseover(function(){methods._prev(".line-container:last",-135,"bottom");});$(".mosaic-next-bottom").mouseover(function(){var max=$(window).width()-$(".line-container:last").width()+100;methods._next(".line-container:last",max,"bottom");});},_prev:function(target,max,origin){var base=parseInt($(target).css("margin-left"));if(isAnimatingTop||isAnimatingBottom||base>max){return false;}
...
if(origin=="top")isAnimatingTop=true;if(origin=="bottom")isAnimatingBottom=true;while(base>max){base-=232;}
$(target).animate({"margin-left":base},600,"easeInOutSine",function(){$(".mosaic-next-"+origin).addClass("disable");$(".mosaic-prev-"+origin).removeClass("disable");if(origin=="top")isAnimatingTop=false;if(origin=="bottom")isAnimatingBottom=false;});}};$.fn.MLMosaic=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.MLMosaic');}};})(jQuery);(function($){var methods={init:function(){var self=this;$(self).click(function(){$.address.value($(this).attr('href'));methods.closeMosaic(this);return false;});},open:function(href,loadContent){$.post(href,{"noLayout":"true"},function(htmlLoaded){$("#content").removeClass("mosaic").addClass("project-detail").html(htmlLoaded);$(window).MLFont("initProject");$(window).MLResize("resize");$("#footer-project").MLFooterProject();if(loadContent){methods._initProject();}else{$(window).MLPreloadImages();}});},openMosaic:function(){$.post("/",{noLayout:true},function(htmlLoaded){$("#content").removeClass("project-detail").addClass("mosaic").html(htmlLoaded).fadeIn(function(){$(window).MLResize("home");$(".mosaic").MLMosaic("display");$(".mosaic ul a").MLOpenProject();$("#menu #work .menu-link").html("work");$("#menu #work .menu-content").html($(".work-content").html());$(window).MLFont();});});},_initProject:function(){$("#content").fadeIn(100,function(){var lis=$("ul.content-project li").length;var startValue=1;$("ul.content-project").jcarousel({start:startValue,scroll:1,wrap:"both",animation:800,easing:"easeInOutSine",itemFallbackDimension:1100,itemVisibleInCallback:{onBeforeAnimation:function(){$(window).trigger("IS_ANIMATING");},onAfterAnimation:function(){$(window).trigger("END_ANIMATING");}}}).animate({"opacity":1},function(){$(window).MLResize("resize");$("#menu").MLMenu("reset");$("#menu a.current").removeClass("current");$("#menu #work a").addClass("current");$("#menu #work .menu-link").html($(".bottom-project-title").html());$("#menu #work .menu-content").html($(".work-content").html());$(window).MLFont("work");$("#menu").MLMenu("open",0);$(window).trigger("INIT_SWF");}).MLImprovedCarousel({carousel:$("ul.content-project").data("jcarousel"),startValue:startValue-1});});},closeMosaic:function(target){$("#content-top").animate({marginTop:228});$("#content-bottom").animate({marginTop:-228},function(){methods.open($(target).attr("href"));});}}
$.fn.MLOpenProject=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.MLOpenProject');}};})(jQuery);(function($){var alreadyLoaded=[];var methods={init:function(options){var self=this,width=$(window).width(),i=1;var imagesArray=alreadyLoaded.length>0?[]:siteImages;if($.address.value()!="/"){var pathNames=$.address.pathNames();var name=pathNames[pathNames.length-1];if(in_array(name,alreadyLoaded)){$(window).trigger("LOAD_COMPLETE",{direct:true});return;}
alreadyLoaded.push(name);$(projectImages[name]).each(function(i,item){imagesArray.push(item);});}else{$(thumbs).each(function(i,item){imagesArray.push(item);});}
$(mini).each(function(i,item){imagesArray.push(item);});var len=imagesArray.length;$(window).trigger("LOAD_STARTED");$.imgpreload(imagesArray,{each:function(){$(window).trigger("LOAD_ITEM",{index:i,width:width,len:len});i++;},all:function(){$(window).trigger("LOAD_COMPLETE",{direct:false});}});}};$.fn.MLPreloadImages=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.MLPreloadImages');}};})(jQuery);(function($){var methods={init:function(options){var self=this;},resize:function(){var marginTop=Math.floor(($(window).height()/2)-394);if(marginTop>0)$("#header").css("margin-top",marginTop);else $("#header").css("margin-top",0);methods.loader(marginTop);methods.background();methods.footerBackground();if($.address.value()=="/"){methods.home();}},loader:function(marginTop){marginTop=marginTop<0?0:marginTop;$("#loader").css({"top":"0","margin-top":marginTop+384});},background:function(){if(!$("#wrapper").hasClass("hasBackground")){$("#wrapper").addClass("hasBackground");return;}
var top=$("#logo").position().top-73;var left=Math.round(($(window).width()-$(".content-inner").width())/2)+13;if($.address.value()!="/")left-=8;$("#wrapper").css({"background-position":(left+"px "+top+"px"),"background-image":"url(/img/site/misc/bg_global_24.png)"});},footerBackground:function(){if(!$("#footer-project ul").position())return;var left=$.browser.msie?parseInt($("#footer-project ul").css("margin-left")):parseInt($("#footer-project ul").position().left);$("#footer-project").css("background-position",(left+60)+"px top");},home:function(){var topWidth=$("#content-top li").length*250+115;var bottomWidth=$("#content-bottom li").length*250+103;$("#content-top").parent().css("width",topWidth);$("#content-bottom").parent().css("width",bottomWidth);var margin="0 auto",marginLast="0 auto";if($(window).width()<$(".content-inner").width()){var marginLeft=Math.round(($(window).width()-$(".content-inner").width())/2);margin="0 0 0 "+marginLeft+"px";marginLast="0 0 0 "+(marginLeft+5)+"px";}
